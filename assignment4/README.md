...
![z\_{\\alpha / 2}=F^{-1}(1-(\\alpha
/ 2))](https://latex.codecogs.com/png.latex?z_%7B%5Calpha%20%2F%202%7D%3DF%5E%7B-1%7D%281-%28%5Calpha%20%2F%202%29%29
"z_{\\alpha / 2}=F^{-1}(1-(\\alpha / 2))")  
  
![C\_{n}=(a, b)=\\left(\\bar{X}-\\frac{\\sigma}{\\sqrt{n}} z\_{\\alpha
/ 2}, \\bar{X}+\\frac{\\sigma}{\\sqrt{n}} z\_{\\alpha
/ 2}\\right)](https://latex.codecogs.com/png.latex?C_%7Bn%7D%3D%28a%2C%20b%29%3D%5Cleft%28%5Cbar%7BX%7D-%5Cfrac%7B%5Csigma%7D%7B%5Csqrt%7Bn%7D%7D%20z_%7B%5Calpha%20%2F%202%7D%2C%20%5Cbar%7BX%7D%2B%5Cfrac%7B%5Csigma%7D%7B%5Csqrt%7Bn%7D%7D%20z_%7B%5Calpha%20%2F%202%7D%5Cright%29
"C_{n}=(a, b)=\\left(\\bar{X}-\\frac{\\sigma}{\\sqrt{n}} z_{\\alpha / 2}, \\bar{X}+\\frac{\\sigma}{\\sqrt{n}} z_{\\alpha / 2}\\right)")  
  
![S E M =
\\frac{\\sigma}{\\sqrt{n}}](https://latex.codecogs.com/png.latex?S%20E%20M%20%3D%20%5Cfrac%7B%5Csigma%7D%7B%5Csqrt%7Bn%7D%7D
"S E M = \\frac{\\sigma}{\\sqrt{n}}")  

First we find ![S E
M(\\hat{k})](https://latex.codecogs.com/png.latex?S%20E%20M%28%5Chat%7Bk%7D%29
"S E M(\\hat{k})") by bootstrap:
...
Than we calculate the 99% confidence interval for the parameter
...
```
    ...
    ## [1] "99% Confidence Interval for k: ( a = 9.97295683209687 , b=  12.8278299775028 )"
...
One side Wald test ![H\_{0} = \\theta
\\leq 10](https://latex.codecogs.com/png.latex?H_%7B0%7D%20%3D%20%5Ctheta%20%5Cleq%2010
"H_{0} = \\theta \\leq 10")
...
![H\_{1} = \\theta
\> 10](https://latex.codecogs.com/png.latex?H_%7B1%7D%20%3D%20%5Ctheta%20%3E%2010
"H_{1} = \\theta \> 10")  

Since ![w \> Z](https://latex.codecogs.com/png.latex?w%20%3E%20Z
"w \> Z") we reject the null hyphotesis, so with 95% confidence we think
that ![k \> 10](https://latex.codecogs.com/png.latex?k%20%3E%2010
"k \> 10").
...
![](assignment4_rmd_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->
...
LRT to test whether the parameter\_rich model is significantly better
than the simpler model.

![H\_{0} =](https://latex.codecogs.com/png.latex?H_%7B0%7D%20%3D
"H_{0} =") the model M1 is sufficient do describe the
data.
...
We can plot the densities of the exp and gamma distributions together
with the spikes:
...
![](assignment4_rmd_files/figure-gfm/unnamed-chunk-11-1.png)<!-- -->

Under the null hypothesis assumption: ![\\lambda\\left(X\_{1}, \\ldots,
X\_{n}\\right)=-2 \\log (q(X)) \\approx
\\chi\_{d-d\_{0}}^{2}](https://latex.codecogs.com/png.latex?%5Clambda%5Cleft%28X_%7B1%7D%2C%20%5Cldots%2C%20X_%7Bn%7D%5Cright%29%3D-2%20%5Clog%20%28q%28X%29%29%20%5Capprox%20%5Cchi_%7Bd-d_%7B0%7D%7D%5E%7B2%7D
"\\lambda\\left(X_{1}, \\ldots, X_{n}\\right)=-2 \\log (q(X)) \\approx \\chi_{d-d_{0}}^{2}").
...
LR test statistic is 33.07 (distributed chi\_squared), with 1 degree of
freedom.

Now we compare
![\\lambda](https://latex.codecogs.com/png.latex?%5Clambda "\\lambda")
with the upper quantile of the chi squared distribution with
![d-d\_{0}](https://latex.codecogs.com/png.latex?d-d_%7B0%7D "d-d_{0}")
degrees of freedom:
...
Since ![\\lambda\\left(X\_{1}, \\ldots, X\_{n}\\right)\>\\chi\_{d-d\_{0}
;
\\alpha}^{2}](https://latex.codecogs.com/png.latex?%5Clambda%5Cleft%28X_%7B1%7D%2C%20%5Cldots%2C%20X_%7Bn%7D%5Cright%29%3E%5Cchi_%7Bd-d_%7B0%7D%20%3B%20%5Calpha%7D%5E%7B2%7D
"\\lambda\\left(X_{1}, \\ldots, X_{n}\\right)\>\\chi_{d-d_{0} ; \\alpha}^{2}")
I reject ![H\_{0}](https://latex.codecogs.com/png.latex?H_%7B0%7D
"H_{0}"). \<- (NOT SURE HERE)

The p\_value =
![1-F\_{\\chi\_{d-d\_{0}}}^{2}\\left(\\lambda\\left(X\_{1}, \\dots,
X\_{n}\\right)\\right)](https://latex.codecogs.com/png.latex?1-F_%7B%5Cchi_%7Bd-d_%7B0%7D%7D%7D%5E%7B2%7D%5Cleft%28%5Clambda%5Cleft%28X_%7B1%7D%2C%20%5Cdots%2C%20X_%7Bn%7D%5Cright%29%5Cright%29
"1-F_{\\chi_{d-d_{0}}}^{2}\\left(\\lambda\\left(X_{1}, \\dots, X_{n}\\right)\\right)")
( in our case
![d-d\_{0}=2-1=1](https://latex.codecogs.com/png.latex?d-d_%7B0%7D%3D2-1%3D1
"d-d_{0}=2-1=1") )
...
P-value is really low (p-value \< alpha?) so we reject the null
hypothesis.

Thus we can state that it is preferable to use the gamma model to
describe the
data.

# Ex 3.

## Ex 3.1 Find the MLE for the exponential, gamma, inverse Gaussian and log\_normal model.
...
![](assignment4_rmd_files/figure-gfm/unnamed-chunk-19-1.png)<!-- -->
...
We choose the model with the smallest AIC or BIC, the inverse normal
distribution fit best our data.

### Now we do the same using functions

First I need a list for each model with the loglikelihood value and
parameters, then I need a list of all models
...
Now we can write the functions for AIC and BIC and apply them to the
list of models (candidates)
...
## Ex 4.1 Estimate the standard error of the MLE estimator of µ for the log\_normal distribution applied to the ramp spike time data.
...
Mll log\_normal distribution and MLE:
...
Calculate ![SE(\\hat{\\log
\\mu})](https://latex.codecogs.com/png.latex?SE%28%5Chat%7B%5Clog%20%5Cmu%7D%29
"SE(\\hat{\\log \\mu})"):
...
```
    ...
    ## [1] "95% Confidence Interval for k: ( a = 1.64362453302169 , b=  1.69404611976528 )"
...
```
    ...
    ## [1] "95% Confidence Interval for k: ( a = 1.78293067818406 , b=  1.9216624354395 )"

2° method (percentile confidence
intervals):
...
## Ex 5.1 Transform the ramp spike time using the logarithm as we did in Week 3 and then perform a two sample t\_test between the human and mouse cells.
...
![H\_{0}:
\\mu\_{h}=\\mu\_{m}](https://latex.codecogs.com/png.latex?H_%7B0%7D%3A%20%5Cmu_%7Bh%7D%3D%5Cmu_%7Bm%7D
"H_{0}: \\mu_{h}=\\mu_{m}")

![H\_{1}: \\mu\_{h} \\neq
\\mu\_{m}](https://latex.codecogs.com/png.latex?H_%7B1%7D%3A%20%5Cmu_%7Bh%7D%20%5Cneq%20%5Cmu_%7Bm%7D
"H_{1}: \\mu_{h} \\neq \\mu_{m}")

p\_value ![\<
\\alpha](https://latex.codecogs.com/png.latex?%3C%20%5Calpha
"\< \\alpha"), we reject
![H\_{0}](https://latex.codecogs.com/png.latex?H_%7B0%7D "H_{0}").

We think that the the 2 means are significantly different with a
confidence of 95% (actually at a level 0.001 from Gherardo solution).

Basic t\_test (var.equal = T) works only with samples that have
homogeneous variance but by default the var.equal is set to FALSE and so
we can use it with sample that have different
variance.

## Ex 5.2 Perform directly a Wald test to check if µh = µm where µh and µm are the mean\_log parameters of the log\_normal distributions for the human and mouse cells.

  
![w =
\\frac{\\left|\\bar{x}\_{h}-\\bar{x}\_{m}\\right|}{\\sqrt{\\frac{s\_{h}^{2}}{n\_{h}}+\\frac{s\_{m}^{2}}{n\_{m}}}}](https://latex.codecogs.com/png.latex?w%20%3D%20%5Cfrac%7B%5Cleft%7C%5Cbar%7Bx%7D_%7Bh%7D-%5Cbar%7Bx%7D_%7Bm%7D%5Cright%7C%7D%7B%5Csqrt%7B%5Cfrac%7Bs_%7Bh%7D%5E%7B2%7D%7D%7Bn_%7Bh%7D%7D%2B%5Cfrac%7Bs_%7Bm%7D%5E%7B2%7D%7D%7Bn_%7Bm%7D%7D%7D%7D
...
![H\_{0}](https://latex.codecogs.com/png.latex?H_%7B0%7D "H_{0}") :
![\\bar{x}\_{h}
=\\bar{x}\_{m}](https://latex.codecogs.com/png.latex?%5Cbar%7Bx%7D_%7Bh%7D%20%3D%5Cbar%7Bx%7D_%7Bm%7D
"\\bar{x}_{h} =\\bar{x}_{m}")

Since ![w \> Z](https://latex.codecogs.com/png.latex?w%20%3E%20Z
"w \> Z") and also, p\_value ![\\leq
\\alpha](https://latex.codecogs.com/png.latex?%5Cleq%20%5Calpha
"\\leq \\alpha"), we reject
![H\_{0}](https://latex.codecogs.com/png.latex?H_%7B0%7D "H_{0}").

## Repeat 5.2 from Gherardo solution

Various calculation to obtain the stadard error of the difference of the
means

Since we assume that
![\\mathbb{V}\\left(X\_{i}\\right)=\\mathbb{V}\\left(Y\_{j}\\right)=\\sigma^{2},](https://latex.codecogs.com/png.latex?%5Cmathbb%7BV%7D%5Cleft%28X_%7Bi%7D%5Cright%29%3D%5Cmathbb%7BV%7D%5Cleft%28Y_%7Bj%7D%5Cright%29%3D%5Csigma%5E%7B2%7D%2C
"\\mathbb{V}\\left(X_{i}\\right)=\\mathbb{V}\\left(Y_{j}\\right)=\\sigma^{2},")
we can write   
![s e(\\delta)=\\sigma \\sqrt{\\frac{m+n}{m
n}}](https://latex.codecogs.com/png.latex?s%20e%28%5Cdelta%29%3D%5Csigma%20%5Csqrt%7B%5Cfrac%7Bm%2Bn%7D%7Bm%20n%7D%7D
"s e(\\delta)=\\sigma \\sqrt{\\frac{m+n}{m n}}")  
and an estimator of ![s
e(\\delta)](https://latex.codecogs.com/png.latex?s%20e%28%5Cdelta%29
"s e(\\delta)") is   
![\\hat{s e}(\\delta)=\\hat{\\sigma} \\sqrt{\\frac{m+n}{m
n}}](https://latex.codecogs.com/png.latex?%5Chat%7Bs%20e%7D%28%5Cdelta%29%3D%5Chat%7B%5Csigma%7D%20%5Csqrt%7B%5Cfrac%7Bm%2Bn%7D%7Bm%20n%7D%7D
"\\hat{s e}(\\delta)=\\hat{\\sigma} \\sqrt{\\frac{m+n}{m n}}")  

where
![\\hat{\\sigma}](https://latex.codecogs.com/png.latex?%5Chat%7B%5Csigma%7D
"\\hat{\\sigma}") is the empirical standard deviation of the joined
sample.
...
![\\text { p-value }=1-2 F\_{Z}(|\\delta| / \\hat{s e}(\\delta))= 2
F\_{Z}(-|\\delta|/ \\hat{s
e}(\\delta))](https://latex.codecogs.com/png.latex?%5Ctext%20%7B%20p-value%20%7D%3D1-2%20F_%7BZ%7D%28%7C%5Cdelta%7C%20%2F%20%5Chat%7Bs%20e%7D%28%5Cdelta%29%29%3D%202%20F_%7BZ%7D%28-%7C%5Cdelta%7C%2F%20%5Chat%7Bs%20e%7D%28%5Cdelta%29%29
...
Also the Wald test obtain a very small p-value and thus a similar result
to the
t-test.
...
## Ex 6.2 Compute the p\_value of the two\_sample t\_test with equal variance.
...
"\> \\alpha"), we can’t reject the ![H\_{0} : µ1 =
µ2](https://latex.codecogs.com/png.latex?H_%7B0%7D%20%3A%20%C2%B51%20%3D%20%C2%B52
"H_{0} : µ1 = µ2"), but the result of the p-value in this case change
from one sample generated to an other.

## Ex 6.3

Compute the Wald test for ![H\_{0} : µ1 =
µ2](https://latex.codecogs.com/png.latex?H_%7B0%7D%20%3A%20%C2%B51%20%3D%20%C2%B52
"H_{0} : µ1 = µ2")
...
The Wald t-test use the
![\\delta=\\bar{X}-\\bar{Y}](https://latex.codecogs.com/png.latex?%5Cdelta%3D%5Cbar%7BX%7D-%5Cbar%7BY%7D
"\\delta=\\bar{X}-\\bar{Y}") statistic. Where ![s e(\\delta)=\\sigma
\\sqrt{\\frac{m+n}{m
n}}](https://latex.codecogs.com/png.latex?s%20e%28%5Cdelta%29%3D%5Csigma%20%5Csqrt%7B%5Cfrac%7Bm%2Bn%7D%7Bm%20n%7D%7D
"s e(\\delta)=\\sigma \\sqrt{\\frac{m+n}{m n}}")

Estimate SE of delta analytically (from the formula above obtained in
5.2)
...
![](assignment4_rmd_files/figure-gfm/unnamed-chunk-43-1.png)<!-- -->

It is visually possible to observe that
![\\delta=\\bar{X}-\\bar{Y}](https://latex.codecogs.com/png.latex?%5Cdelta%3D%5Cbar%7BX%7D-%5Cbar%7BY%7D
"\\delta=\\bar{X}-\\bar{Y}") is Gaussian distributed.

## Ex 6.4

Perform the likelihood ratio test for ![H\_{0} : µ1 =
µ2](https://latex.codecogs.com/png.latex?H_%7B0%7D%20%3A%20%C2%B51%20%3D%20%C2%B52
"H_{0} : µ1 = µ2").
...
## Ex 6.5 Compare the results obtained in the different tests, in particular report the different p\_values.
...
```
    ...
    ##      wald    t.test       LRT 
    ## 0.2567074 0.3332261 0.2567074

For all the test performed the p-value is always larger than
![\\alpha](https://latex.codecogs.com/png.latex?%5Calpha "\\alpha") and
so we don’t reject the
![H\_{0}](https://latex.codecogs.com/png.latex?H_%7B0%7D "H_{0}").
...
Estimate SE\_delta by bootstrap
...
```
    ...
    ##       t.test         wald          LRT 
    ## 1.649588e-07 1.913393e-07 2.670812e-07

With larger sample size the p-values from all tests are really low, thus
we reject Ho. It become evident that
![\\mu\_{1}\\neq\\mu\_{2}](https://latex.codecogs.com/png.latex?%5Cmu_%7B1%7D%5Cneq%5Cmu_%7B2%7D
"\\mu_{1}\\neq\\mu_{2}")
